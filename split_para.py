...
# split data into training/val/testing, only needed for the data with (0,0,0) as goal to generate indicies to pick traj. data points within a file
...
class splitting:
    def __init__(self, timing_ratio, split, target, shape, rep):
        ...
        self.split_indices(data = corrected_data, save_file= './splits/repeats/indices_'+str(int(100*self.timing_ratio))+
                  '_'+str(int(100*self.split))+'_shape_'+self.shape+'_target_'+str(self.target)+'_'+str(int(self.rep))+'.pkl')
        # self.split_indices(data = corrected_data, save_file= '/Users/anjiabei/Documents/research/features/splits/indices_'+str(int(100*self.timing_ratio))+
        #           '_'+str(int(100*self.split))+'.pkl')
...
if __name__ == "__main__":
    ...
    sp = splitting(timing_ratio=args.timing_ratio, split = args.split, target = args.target, shape = args.shape, rep = args.repeat + 50)