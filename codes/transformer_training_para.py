...
class transformer_train:
    ...
    def training_transformer_masked_with_val(
        ...
        checkpoint = tf.keras.callbacks.ModelCheckpoint(
                filepath=f'best_transformer_model_{str(int(100*self.timing_ratio))}_{str(int(self.rep))}.keras',  # can end in .keras
        ...
        # --- Load best weights ---
        model.load_weights(f'best_transformer_model_{str(int(100*self.timing_ratio))}_{str(int(self.rep))}.keras')
        model.save_weights('./model_weights_com/transformer_all_'+str(int(100*self.timing_ratio))+'_'+str(int(self.rep))+'.h5')
...
if __name__ == "__main__":
    ...
    with open(f"./results/evaluation_com/"+str(int(100*args.timing_ratio))+'_'+str(int(100*args.split))+'_'+str(int(args.repeat))+".pkl", "wb") as f: # com as in model is trained in combined corrected and uncorrected data
