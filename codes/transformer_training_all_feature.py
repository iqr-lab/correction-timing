...
# train the transformer for all-feature model
...
class transformer_train:
    ...
    def devide_data(self, ind): #50/50 ind is the length of teh corrected data; for training
        ...
        input_data_all = input_data[:, :, :] # goal related features + optimality
    ...
    def devide_data_val(self): 
        ...
        input_data = np.stack([ self.total_ps_padded_val, self.total_a1_padded_val, self.total_a3_padded_val, 
                                    self.total_dis_padded_val, self.total_a2_padded_val, self.total_legi_padded_val,
                                    self.total_opt_padded_val], axis=-1) # num_datapoints, num_time, num_features
        input_data_all = input_data[:, :, :] # goal-related + optimality
    ...
    def devide_data_test(self): 
        ...
        input_data = np.stack([ self.total_ps_padded_test, self.total_a1_padded_test, self.total_a3_padded_test, 
                                    self.total_dis_padded_test, self.total_a2_padded_test, self.total_legi_padded_test,
                                    self.total_opt_padded_test], axis=-1) # num_datapoints, num_time, num_features
        input_data_all = input_data[:, :, :] # goal-related + optimality
    ...
    def training_transformer_masked_with_val(
        ...
        checkpoint = tf.keras.callbacks.ModelCheckpoint(
                filepath=f'../goal_infer_files/when_weights/temp_models/best_transformer_model_{str(int(100*self.timing_ratio))}_{str(int(self.rep))}.keras',  # can end in .keras
        ...
        # --- Load best weights ---
        model.load_weights(f'../goal_infer_files/when_weights/temp_models/best_transformer_model_{str(int(100*self.timing_ratio))}_{str(int(self.rep))}.keras')
        # model.save_weights('../goal_infer_files/when_weights/transformer_all_'+str(int(100*self.timing_ratio))+'_'+str(int(self.rep))+'.h5')
        model.save_weights('../goal_infer_files/when_weights/model_weights/transformer_all_'+str(int(100*self.timing_ratio))+'_'+str(int(self.rep))+'.weights.h5') # have to change file extention here
...
if __name__ == "__main__":
    ...
    model = tt.training_transformer_masked_with_val() # save weights from training the transformer model
    results = tt.testing(model) # save testing resulys
    with open(f"../results/evaluation/"+str(int(100*args.timing_ratio))+'_'+str(int(100*args.split))+'_'+str(int(args.repeat))+".pkl", "wb") as f: # com as in model is trained in combined corrected and uncorrected data
