...
with open('tasklist_goal_infer.sh','w') as file:
    for tr in [0.7, 0.8, 0.9, 1]:
        for rep in range(10):
            for shape in ['triangle', 'circle', 'square', 'rectangle']:
                for target in range(4):
                        for t in ["grasp", "release"]: # calculate KLD for each case
                    
                            file.write(f'python3 goal_infer.py -s 0.6 -tr {tr} -sh {shape} -tg {int(target)} -r {int(rep)} -t {t}\n')
            ...
            for rep in range(5):
                for shape in ['triangle', 'circle', 'square', 'rectangle']:
                    for target in range(4):
                            ...
                            file.write(f'python3 goal_infer_alpha.py -s 0.6 -tr {tr} -sh {shape} -tg {int(target)} -r {int(rep)} -a {alpha} -t grasp\n') # only for grasp case